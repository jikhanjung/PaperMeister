...
### 진짜 남은 문제 — 그러나 **범위 밖으로 결정**

이 건의 근본 원인은 가드가 아니라 **헤딩 탐지가 목차 줄을 매치한 것**이다. (c)에서 적은
"헤딩 탐지 실패"와 같은 뿌리다 — 한쪽은 못 찾고(→low fallback), 한쪽은 엉뚱한 걸 찾는다.

**사용자 결정 (2026-07-27): 탐지기 정확도 개선은 하지 않는다.** "그런 건 사실 어쩔 수 없지.
일반적인 학술지 논문만 잘 처리해도 오케이야."

근거: 오작동하는 대상이 코스 가이드북·도판·목차·부고·편지처럼 **애초에 인용 네트워크에
기여하지 않는 문서들**이다. 여기에 휴리스틱을 더 쌓으면 규칙만 복잡해지고, 정작 일반
학술지 논문 경로에 회귀 위험을 들인다.

**중요 — 이번 세션의 수정들은 그대로 유지한다.** 성격이 다르기 때문이다:
- 077~079의 가드는 이상 문서를 **잘 처리하려는** 게 아니라, 그것들이 **조용한 유실이나
  무한 루프를 일으키지 못하게 봉쇄**하는 것이다
- 무한 루프는 이상 문서 하나가 **매 배치 선두를 점유**해 일반 논문 처리량을 갉아먹는다
- 즉 봉쇄는 "엣지 케이스 챙기기"가 아니라 **본류를 지키는 비용**이다

후보로 적어뒀던 목차 줄 배제(점선 리더 `.....`+페이지번호), 매치 블록 최소 크기 요구
등은 **보류**. 일반 논문에서 실제 오류가 관측되면 그때 다시 연다.


---

## (e) Treatise류(단행본·합본) 처리 — 검토 후 **보류**

Treatise Part R이 `confidence=low, 10 entries`로 돌면서 **색인 페이지**를 참고문헌으로
변환하는 게 관측됐다(`"family": "Titanocarcinus"`, `"title": "trichobranchia", "pages": "103"`).
논저라 진짜 참고문헌은 장마다 흩어져 있고, fallback이 집어온 마지막 2페이지는 색인이다.

**피해는 갇혀 있음이 코드로 확인됐다.** `canonicalize_reference`가
`if not key and not nd: return None, 'none'` 으로 **제목도 DOI도 없는 항목을 거부**하므로
`CitedWork` 노드가 안 생긴다. resolved_paper·resolved_work 둘 다 null = 이미 정의된 junk
상태로 남는다. **인용 네트워크에 가짜 엣지·노드는 생기지 않는다.** 남는 건 Reference
테이블의 죽은 행과 References 탭의 무의미한 카드뿐이고, 한 번 완료되면 재시도 대상에서
빠지므로 배치 선두를 점유하지도 않는다.

### 검토한 방안과 판단

**방향 전환이 핵심이었다** — Treatise류에서 가치 있는 건 **나가는 인용이 아니라 들어오는
인용**이다. 참고문헌 수백 개를 뽑아야 대부분 미보유 고문헌이라 고립 노드만 늘지만,
"Moore (ed.) 1969, Treatise Part R"을 **인용하는** 논문은 삼엽충 문헌에 흔하고 그게 이
노드를 허브로 만든다. 그리고 들어오는 인용은 **references 추출이 전혀 필요 없다** —
제목·저자·연도만 정확하면 `resolve_references`가 붙여준다(= biblio 파이프라인의 일).

구체안으로 **`confidence == 'low' and len(pages) > ~60` 이면 LLM 호출 없이 checked-empty**를
제안했다. 근거: fallback 휴리스틱은 **짧은 문서에서만 정당**하다. 10쪽 논문의 마지막
2페이지는 참고문헌일 개연성이 높지만, 128쪽 책의 마지막 2페이지는 색인이나 도판이다.
**페이지 수 단독으로 자르면 안 된다** — 이 라이브러리엔 100~200쪽짜리 삼엽충 계통
개정판이 흔한데 그건 진짜 REFERENCES 헤딩이 있어 `high`로 잡히고 지금도 잘 된다.

**사용자 결정 (2026-07-27): "일단은 그냥 두자. 어떻게 처리해도 애매한 것 같아."** → 미구현.

기각하지 않고 **보류**인 이유: 위 규칙은 성격상 (b)(c)의 봉쇄 가드와 같은 계열이라,
쓰레기 행이 실제로 거슬리는 수준이 되면 그때 꺼내 쓰면 된다. 함께 검토했다가 **명시적으로
접은 것**은 장별(chapter) 참고문헌 추출 — 합본을 컨테이너→챕터로 쪼개야 해서 "PDF 하나 =
Paper 하나" 전제를 갈아엎는 일이고, 얻는 것에 비해 너무 크다.
