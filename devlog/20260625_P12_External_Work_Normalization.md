...
**완료 (코드만 — 라이브 DB 미적용):**
- 스키마: `CitedWork`(doi/title/title_key/year/authors_json/container/first_surname/
  cite_count/merge_checked/merged_into_paper), `Reference.resolved_work` FK
  (`models.py`). `database.py` `ALL_TABLES`+마이그레이션(`reference.resolved_work_id`
  컬럼+인덱스, citedwork 테이블은 create_tables 자동 생성).
- 패스1: `references.py`에 `well_formed_doi`/`work_title_key`/`build_work_index`/
  `canonicalize_reference`(exact dedup) + `resolve_paper_references`에 `work_index`
  옵션 추가(하위호환: 없으면 Phase 1 동작). `recompute_cite_counts`,
  `reconcile_works_with_papers`.
- 패스2: `references.py` `cluster_works_by_block`/`merge_works`/`mark_merge_checked`,
