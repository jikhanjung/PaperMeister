...
**28건을 전수 분류**했더니 셋으로 갈렸다.

| 건수 | 정체 | 조치 |
|---|---|---|
| 26 | peewee가 **생성하는** `id` / `<fk>_id` 가 주석에 없음 | `BaseModel`·각 모델에 `TYPE_CHECKING` 선언 |
| 1 | 자기참조 FK를 문자열로 주는 형태가 오버로드에 없음 | 좁은 `type: ignore[call-overload]` |
| 2 | **우리 코드의 타입 느슨함** — 이종 dict | `TypedDict` 도입 |

**`id`를 `int`로 선언하면 안 된다**는 걸 중간에 배웠다. peewee 필드는 디스크립터라
**클래스에서는 쿼리 표현식, 인스턴스에서는 값**이다. `int`로 선언하니 인스턴스 접근 42곳은
통과했지만 `CitedWork.id.in_(dups)` 같은 클래스 접근이 새로 깨졌다. 실제 필드 타입
(`peewee.AutoField`)으로 선언하니 양쪽이 다 맞는다 — peewee 4의 필드에는 `__get__`
오버로드가 제대로 들어 있다.

`<fk>_id`를 `int`로 선언한 건 의도적이다. 인스턴스 접근 42곳이 전부 그렇게 쓰고, 클래스
레벨 `.in_()` 용례는 현재 없다. 나중에 생기면 mypy가 잡아줄 것이고 그때 고치면 된다 —
ratchet이 원래 그렇게 동작해야 한다.

**`x_id` 대신 `x`를 쓰는 "해법"은 절대 안 된다.** mypy는 통과하겠지만 peewee가 관계
객체를 **행마다 새로 fetch**한다. 타입 검사기를 만족시키려다 N+1 쿼리를 심는 셈이다.
`models.py` 주석에 명시해뒀다.

### CI 변경

```yaml
- run: pip install ruff mypy==2.3.0        # 버전 핀 (Modan2 관례)
- name: Install project dependencies (so mypy sees real types)
  run: pip install --require-hashes -r requirements.lock
```

이제 게이트가 **표방하는 일을 실제로 한다.** 검증: 수정 전 코드에 의존성이 깔린 상태로
mypy를 돌리면 **28건이 잡히고**, 수정 후에는 **0건**이다. 이전 CI는 같은 코드에 대해
언제나 0건이라고 답했다 — 그게 문제였다.
