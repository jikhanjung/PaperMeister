...
**공통 규약은 이 저장소 밖에 있다.** 파일 위치·인스톨러 신원·CI·패키징 등 형제 프로젝트가
공유하는 규약은 `.guides/desktop/README.md`가 입구다(`web/`도 있다). `.guides`는 공통 가이드
체크아웃을 가리키는 심볼릭 링크이고, **복사하지 않고 참조만 한다** — 사본은 원본과 어긋나기
때문이다(실제로 하루 만에 그런 적이 있다. devlog 087). 가이드는 비공개이고 이 저장소는
공개이므로 **여기에 커밋하지 않는다.** `.guides`가 없으면 체크아웃이 안 걸린 것이다 —
**끊어진 심볼릭 링크는 빈 디렉터리처럼 보이므로**, 가이드에 내용이 없다고 결론짓기 전에
링크부터 확인할 것.
...
- **용어**: 사용자에게 보이는 곳에서 서지정보 단계는 **Info**(bibliographic information)라고 부른다 — "Biblio"는 코드 안(`PaperBiblio`, `biblio_service`)에만 남긴다(2026-09-22 사용자 결정)
- **파이프라인 상태**: `paper_service.Stages`(ocr/biblio/refs/figs 각 상태 + detail) 한 곳에서 판정 — 목록 Stages 열 배지(`StagesDelegate`), 우클릭 메뉴 분기, Metadata 탭 PROCESSING 카드가 모두 이걸 읽는다. 단계를 더하면 `STAGE_KEYS`·`_STAGE_COLOURS`·메뉴 분기·카드 순으로 (devlog 112)
