...
- **서버 다운 견고성 2건** (사용자 지적):
  ...
  - **서버 다운 시 자동 정지** (`f12b03c`): 연속 incomplete 3회 → references LLM 엔드포인트 직접 핑(`biblio.references_server_alive`, max_tokens=1로 slow≠down 구분) → 죽었으면 큐 비우고 "Stopped — server unreachable", 일시적 blip이면 스트릭 리셋하고 계속. `complete=True`(파싱 or no-ref)가 스트릭 리셋해 오탐 방지. 미완 논문 unchecked라 서버 복구 후 재실행하면 이어짐
