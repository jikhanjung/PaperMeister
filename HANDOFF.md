...
- **새 desktop 앱** (`python -m desktop`, Windows + Anaconda). Phase 3 마무리 + **기본 사용 가능 수준**:
  - 좌측 Rail (Library/Search 모드 + Process/Settings 액션), Zotero 탭 + hierarchical 컬렉션 트리
  - 중앙 PaperList (Status/Authors/Title/Year 컬럼)
  - 우측 DetailPanel — **탭 3개**: Metadata / Biblio(Apply 버튼 포함) / **OCR (markdown 렌더링, sanitized)**
...
- **Phase 4 (hookup)**:
  ...
  - Settings 액션: Rail → PreferencesDialog 연결 완료. 저장 후 Zotero 재동기화 흐름 미검증
...
- [ ] **Settings 액션 실증** — Rail의 Settings 버튼 → PreferencesDialog → 값 저장 후 Zotero 컬렉션 재동기화가 제대로 일어나는지 (기존 `papermeister/ui/main_window.py::_sync_zotero_collections` 동등 동작이 새 desktop 앱에도 필요할 수 있음)
