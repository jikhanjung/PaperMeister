...
- 🟡 (다음 주) **수거 소음**: 휴일 러너 로그에 `collected 2 finished job(s): 0 caption(s)`가 매 패스 반복 — 스킵된 행이 due로 남아(같은 답 digest라 시도는 안 늘어남) 같은 두 잡을 매번 재구성·재적용. 해는 없지만(쓰기 0, 시도 소진 없음) 잡 본문을 매번 다시 받는다. `collect_finished`가 쓰기 0인 잡은 `jobs`로 세지 않거나, 이미 본 (job_id, reply digest)를 기억하도록
