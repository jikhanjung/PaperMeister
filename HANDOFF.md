...
**Phase: P07 Phase 2 완전 종료 / Phase 3 스캐폴드 완료 + 쉘 실증 / Phase 4 진행 중 / Phase D 대기**
...
- **새 desktop 앱 쉘** (`python -m desktop`, Windows + Anaconda 환경에서 첫 실행 검증) — Rail(Library/Search 모드 + Process/Settings 액션) / SourceNav 탭 / PaperList 컬럼 / 트리 chevron 등 쉘 레벨 UI 버그 정리 완료

### 진행 중인 것
- **Phase 3 (desktop)**: 쉘 수준은 Windows에서 실제 동작 확인됨. 아직 미검증: Apply Biblio 버튼 클릭, Process 액션 end-to-end (pending 논문에 실제 OCR 트리거), Settings 저장 후 Zotero 재동기화 흐름
...
- [ ] **Apply Biblio 버튼 클릭 실증** — 백엔드는 검증됨. Needs Review 필터에서 논문 선택 → Apply 눌러서 Zotero write-back까지 가는지 실제로 클릭 (세션 11에서 쉘은 Windows 네이티브 실행 완료)
- [ ] **Process 액션 end-to-end 검증** — pending 논문이 있는 상태에서 Rail의 Process 버튼 → 확인 다이얼로그 → `ProcessWindow`가 실제 OCR 돌리는지 + status bar 카운트가 갱신되는지
- [ ] **Settings 액션 실증** — Rail의 Settings 버튼 → PreferencesDialog → 값 저장 후 Zotero 컬렉션 재동기화가 제대로 일어나는지 (기존 main_window.py의 `_sync_zotero_collections` 동등 동작 필요할 수 있음)
