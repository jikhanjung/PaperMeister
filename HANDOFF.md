...
- **새 desktop 앱** (`python -m desktop`, Windows + Anaconda):
  ...
  - **로컬 폴더 가져오기** (세션 47): Rail 'import' 액션(아이콘 `import.svg`) → `QFileDialog`로 폴더 선택 → `BackgroundTask`로 `ingestion.import_source_directory`(재귀 스캔 + SHA256 dedup, directory Source 생성) → SourceNav refresh(폴더명 탭 등장) → 그 소스 pending PDF "지금 OCR 처리?" 확인(구 GUI 동작) → `ProcessWindow` 재사용. 멀티탭(Zotero + 폴더1/2…)은 SourceNav가 이미 소스마다 탭이라 공짜. 로컬 PDF는 다운로드 없이 PyMuPDF 메타 추출, biblio도 동일(write-back/JSON/promote만 비활성). **end-to-end는 사용자 Windows 검증 대기**. [devlog 062](./devlog/20260615_062_Desktop_Local_Folder_Import.md)
