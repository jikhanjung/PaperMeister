...
⚠️ **5xx 재시도는 이 유형을 못 잡는다** — 72회 발동 / 24회 소진 = 3:1, 즉 **24건 전부
재시도 2회(5s+15s)를 다 쓰고 실패**했다. 엔진이 20초 안에 안 돌아온다. 짧은 blip용 설계라
장기 크래시엔 무력한 게 맞고, **손실은 없다**(해당 논문은 unchecked로 남아 재시도).
필요해지면 backoff를 늘리는 대신 `ServerGuard`의 pause 임계값을 낮추는 쪽이 맞다.
