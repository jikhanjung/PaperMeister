...
- **evaluate confidence 완화 (stub/placeholder는 medium 허용)**: 기존엔 `confidence != high`면 무조건 `low_confidence` needs_review라 promoted standalone(title=파일명)도 medium이면 검토로 빠짐. stub(`_is_stub_paper`) 또는 title이 파일명 placeholder(`zotero_writeback.title_is_filename_placeholder`, public화)면 medium도 게이트 통과 → auto_commit. low는 여전히 보류, curated 논문은 high 유지
