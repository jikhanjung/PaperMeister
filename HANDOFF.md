...
- [ ] **[P16](./devlog/20260914_P16_Figure_Panel_Split.md)** — 논문 도판·플레이트를 소패널 단위로:
  ...
  - ✅ **Phase 4b (앱)** (2026-09-18, [106](./devlog/20260918_106_P16_Process_Figures_In_The_App.md)): 우클릭 "Process Figures"(논문/폴더/전체) → `figure_pipeline.process_file`이 ①→①′→②→③을 **한 논문씩** 직렬 큐로(끝난 단계는 키로 건너뜀, 단계마다 JSON 갱신) + `FiguresWindow`(워커 paused = "Waiting", 취소는 다음 단계에서) + 서버 미설정이면 **비활성 + 이유 툴팁**. 537 passed.
    - ✅ (09-22) **앱을 닫아 두고 온 잡 수거**: `figure_pipeline.collect_finished()`(레인 `--collect`의 라이브러리판)를 **앱 시작 때·Process Figures 시작 직전**에 백그라운드로 실행 → 서버에 끝나 있는 link/panels 답을 DB에 반영하고 배지 갱신. detect 답은 아직 레인 `detect_figures.py --collect`로만
  ...
  - ✅ **Figures 탭** (2026-09-21, [109](./devlog/20260921_109_P16_Figures_Tab.md)): Text와 References 사이. 도판/플레이트 블록을 페이지 순으로 — 도판 전체(≤760px) + 서브피겨 상자, 아래 캡션 항목(패널 색). **상자 hover → 항목 노랑 채움, 항목 hover → 상자 노랑 강조·나머지 흐림**, 클릭은 고정. 렌더는 뷰포트 근처만(600px 선행, 2,400px 밖 해제, 최대 10장 보유). conftest에 `papermeister.*` 재import 복원 픽스처 추가(전체 실행에서만 나던 "uninitialized Proxy")
    ...
    - ✅ **③ 파일럿 전체 완료**(09-22 수거): 253장 **전부 written, 실패 0, 거부 0**. 누계 **35편 379장, 패널 6,440**(fossil_plate 357·diagram 10·photo 6·map 3·chart 2·mixed 1; compound 377). 검토 사유 15건뿐: `entry_on_several_panels` 14(사진+선화·복수 뷰), `panel_count_out_of_range` 1(#114). 다음은 사람이 Figures 탭에서 훑는 ③ 게이트 판단 → 통과면 나머지 파일럿 link(~70편) → panels 순. ⚠️ panels 레인은 **같은 PDF 형제를 해시로 합치지 않는다**(Lee 2004 ×3, Müller 2010 ×2 → 11장 중복) — link의 `propagate_link` 같은 처리가 필요
  ...
  - ✅ **머신 간 도판 동기화 양방향** (2026-09-21, [110](./devlog/20260921_110_P16_Cross_Machine_Figure_Sync.md)): (1) 캐시 JSON의 `exported_at`이 바뀌면 행이 있어도 import(`import_from_cache_if_new`), (2) **Zotero 증분 sync가 돌려준 JSON 첨부의 md5가 로컬 캐시와 다르면 그 자리에서 재다운로드 + figures import**(`ingestion._refresh_sibling_json`). 로컬 디렉터리 소스만 쓰면 원래 해당 없음
  ...
  - ✅ **v0.2.0 릴리스**(2026-09-22): CHANGELOG `[0.2.0]` + ko 카탈로그 + `version.py` 0.2.0 + 태그. beta 대신 정식으로 — Phase 4까지 끝났고 서버 없는 환경에선 Process Figures가 이유와 함께 비활성화되므로. `release.yml`이 3플랫폼 빌드
    - ✅ **1189/1190 새 프롬프트 재실행 결과(09-22)**: 24항목(플레이트 2장씩) 전부 done, 실패·거부 0, **48/50 linked, 항목 1,756개**(옛 답 17 linked, `skipped:other` 31 → 0), 맨 숫자 이름 0(`Pl. 12.` 형태), 열람 페이지 항목당 5–11쪽(옛 231쪽 전체). 항목당 156–648 s, 합 9,397 s. 검토 1건(`plate_no_entries` #174). 프롬프트 수정은 **품질에서 확실히 이득** — 파일럿 전체 재실행 할 만함
    - ✅ **읽기 범위(reading set)** (09-22, [113](./devlog/20260922_113_Link_Reading_Set.md)): 1차 시도는 도판 ±2쪽 + 설명 페이지 + 플레이트 묶음 앞 블록 + 이 논문의 기존 설명 페이지만 담은 **작은 workspace**(자기 digest), 실패마다 넓힘(2차: 지정어 언급·번호 캡션 페이지, 3차: 전체). 모노그래프 tier0 21–31쪽/218–291쪽. 서버 변경 없음. collect는 digest 대신 파일+버전으로 답을 맞춤. 프롬프트 `link-v1-533479a75b9f`(workspace가 일부 페이지일 수 있다는 안내). ⚠️ 실전 측정 전 — 1189 수거 뒤 큰 논문 한 편으로 시간 비교
...
- ✅ (09-22, [112](./devlog/20260922_112_Pipeline_Stages_Badges_And_Menu.md)) **파이프라인 4단계(OCR→Bibliography→References→Figures)를 상태로 정리**: 목록 Status 열은 **현재/다음 단계 배지 하나**(`OCR wait`·`INFO rev`·`REF part`·`FIG cap`·`done`, hover에 4단계 표), 옛 Status pill 제거, Biblio 단계 이름은 **Info**. 우클릭 메뉴가 단계 상태를 따라감(OCR 전엔 OCR만; Extract/Review/Re-extract; Process Figures 라벨이 다음 할 일), Metadata 탭 PROCESSING 카드
- ✅ (09-22) **프레임리스 메인 창**: OS 타이틀바 제거, 상단 바가 타이틀바(드래그 이동 `startSystemMove`, 더블클릭 최대화, 최소/최대/닫기 버튼), 버전은 앱 이름 옆 `v0.1.9`, 가장자리 6px로 리사이즈(`startSystemResize`). `desktop/components/window_chrome.py`. 되돌리려면 preferences.json에 `"native_title_bar": true`(UI 토글은 아직 없음). ⚠️ Windows 실물에서 최대화 시 작업표시줄 가림·스냅 동작은 확인 필요
- 🔴→✅ (09-25) **휴일 러너가 이틀간 거의 멈춰 있던 원인**: 한 논문(14, 두 번째 PDF)의 패널 2장이 서버 큐에서 기다리는 동안 DB에선 계속 "due"라 **5분마다 다시 제출** → 50잡·100항목이 큐를 채워 러너가 "가득 참"으로 보고 새 작업을 안 넣음 → 컬렉션 1은 40편만 link. 또 panels 후보가 러너가 link한 논문뿐이라 **파일럿 재link 결과(3,106도판)의 panels는 아예 안 나감**. 고침(`1e622d7`): 서버에 이미 있는 항목 키는 재제출 안 함, panels 후보 = 캡션 있고 미분할인 모든 파일, 수거는 settled 잡을 기억해 건너뜀, 클라이언트 `cancel`. 중복 50잡 취소, 러너 재시작(PID 22192, `--days 4`, 옛 로그는 `figure_queue.0923.*`) → 첫 패스에 컬렉션 1 link 59편 + panels 8파일
- ✅ (09-25) 수거 소음 — settled 잡 건너뛰기로 해결(위)
- ~~(다음 주) **수거 소음**~~ 해결: 휴일 러너 로그에 `collected 2 finished job(s): 0 caption(s)`가 매 패스 반복 — 스킵된 행이 due로 남아(같은 답 digest라 시도는 안 늘어남) 같은 두 잡을 매번 재구성·재적용. 해는 없지만(쓰기 0, 시도 소진 없음) 잡 본문을 매번 다시 받는다. `collect_finished`가 쓰기 0인 잡은 `jobs`로 세지 않거나, 이미 본 (job_id, reply digest)를 기억하도록
- 🟡 (다음 주) **fsis P47 `pipeline_tick.py`에서 가져올 것**: 오래 사는 러너 대신 **5분 틱**(작업 스케줄러 + 잠금 + 틱 예산) — 러너가 죽어 몇 시간 멈추는 일이 구조적으로 없어짐 / 논문 단위 `--status` 표 / 서버 `paused_reason` 백오프
- 🟡 (다음 주) **reading set 확장 방아쇠에 "얇은 답" 추가** — 지금은 실패(스킵·거부·무응답)만 시도로 세서 넓히는데, 좁은 범위에서 본문 인용을 잡아 "성공"하면(fsis devlog 261의 함정) 다시 넓어지지 않는다. `plate_no_entries`·`caption_shared`·`entries_shrank`면 기록은 하되 시도 1회로 쳐서 다음 실행이 한 티어 넓게 묻도록. **휴일 러너 재시작 때 반영 여부 결정**. 가이드 §2.3에 역사(좁게→전체→단계적 확장) 기록함
- 📘 (09-23) **[`docs/figure_pipeline_guide.md`](./docs/figure_pipeline_guide.md)** — P16에서 배운 것 전부를 한 문서로: 단계 설계, workspace로 단계 잇기와 reading set, 키/재개, 제출·수거 분리, 답 크기와 분할, 검증 기준, 머신 간 동기화, 검수 UI, 무인 운영과 사고, 측정값, 다시 만든다면의 순서
- 🔴→✅ (09-23) **무인 실행 첫 시도가 4시간 만에 죽은 원인**: `figure_queue.py`가 `install_system_trust()`를 안 불렀다 — link 잡이 처음 끝나 수거 → 캐시 JSON 기록 → Zotero sibling 푸시에서 기관망 TLS로 `CERTIFICATE_VERIFY_FAILED`, 미처리 예외로 프로세스 종료(stderr를 리다이렉트 안 해서 로그에도 안 남음). 고침: 러너가 트러스트 설치 + 패스 단위 예외 격리, `figure_share.write_to_cache`는 푸시 실패를 경고만 하고 로컬 기록은 유지. **재시작 시 stderr도 남길 것**(`-RedirectStandardError`)
- 🟡 (09-23~) **도판 파이프라인 무인 운영**: 파일럿 재link 165항목(00:40 제출, ~18 h) → 이어서 `scripts/figure_queue.py`가 **Zotero 컬렉션 트리 순서**(0. Antarctic Trilobite → Antarctic Taxonomy → Antarctica archaeocyath → …)로 assemble+link+panels를 큐에 계속 채움. 200쪽 초과 PDF 제외(초록집), 큐 120항목 유지, 상태는 `<data>/figure_queue.json`. 실행: `python scripts\figure_queue.py --days 6 --execute`(앱 닫고). 중단은 `<data>\figure_queue.stop` 파일 생성. SQLite `busy_timeout=15s` 추가로 앱과 겹쳐도 바로 실패하지 않음
