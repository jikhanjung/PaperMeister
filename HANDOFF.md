...
- **새 desktop 앱** (`python -m desktop`, Windows + Anaconda):
  ...
  - **Sibling JSON pre-fetch**: OCR 진입 시 로컬 cache miss + Zotero-sourced이면 같은 paper의 `{pdf_hash}.json` sibling을 DB로 확인 → 있으면 Zotero에서 다운로드해 cache에 atomic write → OCR API call 우회 (크로스머신/캐시 손실 복구)

### 진행 중인 것
- **Phase 4 (hookup)**:
  - **Apply Biblio Zotero write-back**: 세션 17에서 pref 토글로 명시화 + 403 우아한 처리 + evaluate 버그 두 건 픽스. 사용자가 write 권한 키로 갱신했고 실제 PATCH end-to-end 라이브 검증 진행 중
...
- **1960s standalone OCR**: 226편 상태 미확인
...
- [ ] **Apply Biblio Zotero write-back 라이브 검증** — 세션 17에서 토글 + 403 핸들링 + evaluate 정규화 버그 픽스. 사용자가 write 권한 키 갱신. 다음 세션에 paper 2243 (Rode et al 2003) 같은 케이스로 already_complete 확인 + auto_commit 한 건이라도 Zotero version 증가 확인
