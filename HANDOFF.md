...
- **새 desktop 앱** (`python -m desktop`, Windows + Anaconda):
  ...
  - **Extract Biblio**: 우클릭 → Sonnet 4.6으로 서지 추출, 자동 apply (Zotero 데이터 일치 시)
  - **PDF 캐시**: `~/.papermeister/pdf_cache/{zotero_key}/{filename}` — Zotero 다운로드 캐싱

### 진행 중인 것
- **Phase 4 (hookup)**:
  - **Apply Biblio 대조 비교 UI 완성** (세션 14): 필드별 라디오 선택 + 편집 가능 + apply_merged() 연결. NameError 버그 수정됨
  - **Apply Biblio Zotero write-back hookup** (세션 15): `apply_merged()`이 Zotero-sourced paper에서 `zotero_writeback.writeback_overrides()` 경로를 타도록 분기. 사용자가 비교 UI에서 고른 값은 user explicit choice 정책으로 Zotero 값을 덮어쓴다 (빈 슬롯 fill 아님). 저자는 firstName/lastName split. dry-run 4케이스 통과. **실제 라이브 write end-to-end 검증은 미완**
...
- [ ] **Apply Biblio Zotero write-back 라이브 검증** — 코드 hookup 완료 (세션 15). desktop UI에서 Zotero-sourced paper 비교 UI → Apply → Zotero 서버 item 실제 업데이트 확인 + local mirror refresh 확인 필요. dry-run은 통과
