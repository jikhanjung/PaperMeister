...
- **코드 품질 인프라 도입 (P15, ../Modan2 가이드 기반)** — [R01 검토](./devlog/20260723_R01_Code_Quality_Guide_Adoption.md) · [P15 계획](./devlog/20260723_P15_Code_Quality_Adoption_Plan.md)
  ...
  - **다음 Phase 3 (CI)**: GitHub Actions 매트릭스(**Windows 러너 필수**) + import 스모크 게이팅. **사용자 요청: `../Modan2/.github/workflows` 조사해 서버 패키징·릴리스 방식 맞추기** (Phase 3 착수 시)
  - 이후 Phase 4(전역 excepthook+pre-commit)/5(deps·mypy·vulture)/6(패키징 스모크·인코딩). `ruff format`은 별도 커밋으로 미실행
