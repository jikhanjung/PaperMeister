...
- **새 desktop 앱** (`python -m desktop`, Windows + Anaconda):
  ...
  - **Preferences About 섹션**: 하단에 read-only `Client ID` 표시
...
- [ ] **48편 extracted 잔존분 재시도** — 세션 18 폴더 처리 중 bookSection 400으로 멈춘 케이스 + needs_review 정상 케이스 혼재. 세션 19에서 같은 폴더들 다시 Process 한 번 돌려서 `ITEM_TYPE_JOURNAL_FIELD` 픽스 효과 + biblio_state 메타 cross-machine sync 확인
