...
- **서버 다운 견고성 2건** (사용자 지적):
  ...
  - **OCR 후속 (사용자 요청, 대기)**: OCR도 무인 장시간 실행이라 같은 pause/resume 필요(현재는 failed pill만—누군가 봐야 조치). 단 **동결 ProcessWindow + 병렬 ThreadPoolExecutor**라 구조가 달라 `ServerGuard`를 그대로는 못 얹음(직렬 큐 전제). health_check는 `ocr.is_ready` 재사용 가능. **biblio 완료 후 별도 재검토 예정**
