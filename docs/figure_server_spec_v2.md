...
- **도판이 많은 논문은 항목 여럿으로 나뉜다**(2026-09-17, Balašova 1976 도판 91: 세 세션 모두 답을 쓰다 끊김). 클라이언트가 due 도판을 쪽 순서로 **40개 이하**씩 묶어
  같은 잡의 항목으로 보낸다 — 키 `…@<version>#i/n`, `part: [i, n]`, 잠긴 context 행은 모든 항목에. 항목마다 세션이 따로 돌므로 답이 작아진다. 서버 변경 없음.
